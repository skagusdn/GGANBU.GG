    ...
    <>
      ...
      {gameStart === true && timer < 0 &&
        <div className={styles.container} >
          ...
          <span className={styles.msg}>{score < 10 && score >= 5 && " 롤을 해보긴 하셨군요."}</span>
          <span className={styles.msg}>{score < 15 && score >= 10 && " 상당한 실력의 소유자시군요!"}</span>
          <span className={styles.msg}>{score >= 15 && " 롤을 그만하셔야 될 것 같습니다!"}</span>
          <button className={styles.btn} onClick={() => {
            setGameStart(false);
            setScore(0);
            setTimer(60);
