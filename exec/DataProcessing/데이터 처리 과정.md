...
### 사용 방법
> collectUtil에서 api_keys에 라이엇 개발자 페이지에서 발급받은 api 키들을 세팅
> collectUtil의 result_save_folder에 결과 저장 디렉토리 지정
> collectMatchId.py의 con 정의에서 사용할 mysql 스키마 및 사용자 지정, 해당 db에 match_id라는 문자열 컬럼 하나를 match_ids 테이블 생성.
> collectingMain.py에서 cnm.collect_summoner_names에 수집할 티어와 페이지 입력.
